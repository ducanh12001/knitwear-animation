...
interface UseCookiesReturn {
  ...
  setCookie: (name: string, value: any, options?: CookieOptions) => boolean;
...
const useCookies = (): UseCookiesReturn => {
  const [cookies, setCookies] = useState<CookieValue>({});
  ...
  const parseCookies = useCallback((): CookieValue => {
    if (typeof document === 'undefined') return {};

    try {
      return document.cookie
        .split(';')
        .reduce((acc: CookieValue, cookie: string) => {
  ...
  const setCookie = useCallback(
    (name: string, value: any, options: CookieOptions = {}): boolean => {
      ...
      try {
        ...
        // Update local state
        setCookies((prevCookies: CookieValue) => {
